...
def snap_short_timestamps_to_sentences(transcription: str, start_time: str, end_time: str) -> tuple[str, str]:
    """
    محاذاة توقيت البداية والنهاية تلقائياً لأقرب حدود جملة طبيعية ومكتملة 100%،
    ...
    for seg in segments:
        if (seg["start"] <= start_sec <= seg["end"]) or (abs(seg["start"] - start_sec) <= 2.5):
            # نبدأ قبل أول كلمة بهامش أمان 0.25s حتى لا يُبتَر الحرف الأول
            snapped_start = max(0.0, seg["start"] - 0.25)
            break

    # 2. محاذاة النهاية لآخر جملة كاملة حتى لا يتم قطع المتحدث قبل أن يختم كلمته وفكرته
    snapped_end = end_sec
    
    # ابحث عن أي جملة تتقاطع مع end_sec
    intersecting_idx = -1
    for i, s in enumerate(segments):
        if s["start"] <= end_sec <= s["end"]:
            intersecting_idx = i
            break

    if intersecting_idx != -1:
        chosen_seg = segments[intersecting_idx]
        # إذا كانت الجملة الحالية تنتهي بعد end_sec، نمد النهاية حتى نهايتها بالكامل
        snapped_end = chosen_seg["end"]
        # فحص إذا كانت الجملة تنتهي بكلمة استدراكية أو عطف مثل (و، ثم، لكن، لأن، فقال، عشان)
        text_clean = chosen_seg.get("text", "").strip()
        trailing_conjunctions = ("و", "ثم", "لكن", "لأن", "فقال", "عشان", "علشان", "بس", "مع ذلك", "يعني", "لما")
        if any(text_clean.endswith(c) or text_clean.endswith(c + "...") for c in trailing_conjunctions):
            if intersecting_idx + 1 < len(segments):
                snapped_end = segments[intersecting_idx + 1]["end"]
    else:
        # ابحث عن الجمل الواقعة قبل end_sec
        matching_segs = [s for s in segments if s["start"] >= (snapped_start - 0.5) and s["start"] <= (end_sec + 1.5)]
        if matching_segs:
    ...
    # إضافة هامش أمان صوتي ختامي (+0.6 ثانية) لضمان أن المتحدث أنهى نبرة الكلمة الأخيرة بنعومة وهدوء
    snapped_end += 0.6
...
def call_openrouter_shorts(transcription: str, num_shorts: int, api_key: str, model_name: str = "google/gemini-3.5-flash-lite", custom_prompt: str = None, title_style: str = "auto"):
    ...
    system_prompt = (
        "أنت خبير ومخرج ومونتير محترف عالمياً في صناعة واقتطاع المحتوى الفيروسي الأكثر انتشاراً وتأثيراً على TikTok و Shorts و Reels (Viral Content Creator & Master Storyteller).\n"
        "مهمتك الأساسية والجوهرية: تحليل النص المفرغ واستخراج مقاطع قصيرة (Shorts) مميزة، متكاملة الأركان 100% (بداية مشوقة + سياق قوي + كشف السر/الإجابة + قفلة ختامية تامة ومقنعة بدون أي بتر أو قطع مفاجئ).\n"
        "\n"
        "👑 القواعد الذهبية لمنع المقاطع المبتورة وضمان القفلة المكتملة 100% (Complete Narrative & Zero-Cutoff Policy):\n"
        "1. اكتمال الوعد والسر المطروح في البداية (Payoff Guarantee): كل مقطع شورتس هو قصة قائمة بذاتها. إذا طرحت البداية لغزاً أو سؤالاً أو سر نجاح، يجب أن يشمل المقطع الشرح وكشف السر والأرقام والنتيجة بالكامل حتى آخر كلمة.\n"
        "2. ممنوع منعاً باتاً استخراج 'المقدمات التشويقية فقط' (No Intro-Only Teasers): يُمنع قص مقطع ينتهي بعبارات تحيل المشاهد للمستقبل (مثل: 'وده اللي هنعرفه'، 'تعالوا نشوف إيه اللي حصل'، 'فقال لي السر وهو...') دون أن يحتوي المقطع على الجواب والخاتمة التامة!\n"
        "3. القفلة الختامية المحكمة (Solid Punchline & Resolution): يجب أن ينتهي المقطع دائماً عند نهاية جملة تامة مكتملة الأركان (نقطة ختامية / عبرة / نصيحة حاسمة). ممنوع إنهاء المقطع على كلمة عطف أو استدراك (مثل: 'ولكن...'، 'وبعدين...'، 'عشان...') أو في منتصف نطق المتحدث.\n"
        "4. مد التوقيت لضمان الاكتمال (Generous End Time): إذا كانت الفكرة تحتاج إلى 10 أو 20 ثانية إضافية لتكتمل قفلتها، قم بمد توقيت النهاية (end_time) حتماً حتى انتهاء الجملة الأخيرة بالكامل.\n"
        "5. تصنيف نوع المقطع (category): صنف كل مقطع (🎯 قصة وخلاصة مكتملة / 💡 سر ونصيحة ذهبية / 🔥 موقف درامي / صدمة / ⚡ مقطع تشويقي).\n"
        "\n"
        "⚠️ شروط العنوان وتصحيح الأخطاء اللغوية الإلزامية:\n"
        "1. التدقيق اللغوي والنحوي الصارم: صياغة العنوان بلغة عربية فصيحة وسليمة نحوياً وإملائياً 100%، وتصحيح أي أخطاء لغوية في التفريغ الصوتي.\n"
        "2. الارتباط العميق بمحتوى المقطع وإثارة الفضول القاتل (Curiosity Hook).\n"
...
async def suggest_shorts(req: SuggestShortsRequest):
    ...
    if not shorts_list:
        ...
        try:
            ...
            prompt = (
                "أنت خبير ومخرج ومونتير محترف عالمياً في صناعة واقتطاع المحتوى الفيروسي الأكثر انتشاراً وتأثيراً على TikTok و Shorts و Reels (Viral Content Creator & Master Storyteller).\n"
                f"قم بتحليل النص المفرغ التالي واستخرج منه أفضل {req.numShorts} مقاطع قصيرة مميزة، متكاملة الأركان 100% (30 إلى 180 ثانية كحد أقصى):\n\n"
                "👑 القواعد الذهبية لمنع المقاطع المبتورة وضمان القفلة المكتملة 100% (Zero-Cutoff Policy):\n"
                "1. اكتمال الوعد والسر المطروح في البداية (Payoff Guarantee): كل مقطع شورتس هو قصة قائمة بذاتها. إذا طرحت البداية لغزاً أو سؤالاً أو سر نجاح، يجب أن يشمل المقطع الشرح وكشف السر والأرقام والنتيجة بالكامل حتى آخر كلمة.\n"
                "2. ممنوع استخراج مقدمات الفيديوهات التشويقية التي تحيل المشاهد للمستقبل دون كشف الجواب والسر في المقطع نفسه.\n"
                "3. القفلة الختامية المحكمة (Solid Punchline & Resolution): يجب أن ينتهي المقطع دائماً عند نهاية جملة تامة مكتملة الأركان (نقطة ختامية / عبرة / نصيحة حاسمة).\n"
                "4. مد التوقيت لضمان الاكتمال (Generous End Time): إذا كانت الفكرة تحتاج إلى 10 أو 20 ثانية إضافية لتكتمل قفلتها، قم بمد توقيت النهاية (end_time) حتماً حتى انتهاء الجملة الأخيرة بالكامل.\n\n"
            )
...
def cut_segment_fast(url: str, start_sec: float, end_sec: float, quality: int, output_path: str, progress_callback=None) -> str:
    ...
    start_fade_out = max(0.5, clip_len + 0.3)
